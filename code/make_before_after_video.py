...
same reference motion (the choreography extended with a settle-to-stance tail,
325 control steps @ 50 Hz, every termination except time_out disabled, so no
episode reset can hide a fall). The plot tracks each policy's root height
against the reference band; the numbers panel is computed from the track files
...
CAPTIONS = [
    (0.00, 0.24, "Both policies are given the same reference: the ATL Stomp at its original tempo, then a settle to a finish."),
    (0.24, 0.55, "Both track the routine - hops, spins, a deep squat - then the base model loses balance."),
    (0.55, 0.80, "AFTER rides through the same moment and eases back to its stance."),
    (0.80, 1.01, "AFTER finishes every step upright and ends standing at rest."),
]
