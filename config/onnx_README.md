...
The stock SONIC base model fails this move: in the paired rollout below it
loses balance and falls (5.06 s in), ending on the floor. The released policy
completes **the whole routine in free dynamics without falling** and finishes
standing at rest, on both the original-tempo clip and the 1.5× slowed variant
it was trained on.

## Results

Free-dynamics rollout on the same reference — the original-tempo clip plus a
settle-to-stance tail (550 control steps @ 50 Hz) — every episode termination
...
The same policy on the 1.5× slowed reference plus a settle-to-stance tail runs
all 544 control steps — root-height minimum 0.663 m, mean tracking error
